...
<div className="my-8 p-6 bg-gradient-to-br from-green-900 to-teal-900 rounded-xl shadow-lg">
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div className="bg-black/30 p-4 rounded-lg">
      ...
      <p className="text-sm text-gray-300 mt-2">8 tokens → 26 characters</p>
      <p className="text-sm text-green-300 font-semibold">Compression: 38%</p>
    </div>
    <div className="bg-black/30 p-4 rounded-lg">
      <p className="text-white font-mono text-lg">"Basketball Legend..."</p>
      <p className="text-sm text-gray-300 mt-2">26 tokens → 68 characters</p>
      <p className="text-sm text-green-300 font-semibold">Compression: 38%</p>
    </div>
    <div className="bg-black/30 p-4 rounded-lg">
      <p className="text-white font-mono text-lg">"General Text (Taylor Swift)"</p>
      <p className="text-sm text-gray-300 mt-2">229 tokens → 410 characters</p>
      <p className="text-sm text-yellow-300 font-semibold">Compression: 56%</p>
    </div>
    <div className="bg-black/30 p-4 rounded-lg">
      <p className="text-white font-mono text-lg">Vocabulary Size: 512</p>
      <p className="text-sm text-gray-300 mt-2">256 base + 255 merges + 1 special</p>
      <p className="text-sm text-blue-300 font-semibold">Efficient & Compact</p>
    </div>
  </div>
...
```
Iteration 2: Most frequent pair is now (s,o) → "so" gets ID 258
Iteration 3: (J,ay) → "Jay" gets ID 259
Iteration 4: (Jay,s) → "Jays" gets ID 260
Iteration 5: (Jays,o) → "Jayso" gets ID 261
Iteration 6: (Jayso,n) → "Jayson" gets ID 262
...and so on
```
...
1. **"Jayson"** → Token 479 (the star gets his own token!)
2. **"Tatum"** → Token 318 (appears constantly)
3. **"Celtics"** → Token 380 (team loyalty!)
4. **"basketball"** → Token 397 (the sport itself)
5. **" the"** → Token 294 (still needed common English)
...
class BPE_Tokenizer:
    def __init__(self):
        self.vocab = {}          # Token ID → bytes mapping
        self.merges = {}         # Pair → Token ID mapping
...
**Testing & Validation:**
- Round-trip testing (encode → decode → original text)
